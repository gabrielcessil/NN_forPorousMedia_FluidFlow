...
def find_highest_vis_folder(base_folder):
    ...
    if vis_folders:
        ...
        return os.path.join(base_folder, highest_folder)
    else:
        raise FileNotFoundError("No 'vis+number' folders found in the specified directory.")
...
def Run_Example(simulations_main_folder,
    ...
    # LBM run
    lbm_command = f"{lbm_file_path}/mpi/bin/mpirun -np 1 {lbm_file_path}/ /bin/{lbm_functional} {simulation_name}.db"
    ...
    vis_folder = find_highest_vis_folder(file_path_simulation)
    silo2vti_command = f"{file_path_silo2vti_exe} summary.silo output.pvti"
    
    # Convert Silo output file into vti
